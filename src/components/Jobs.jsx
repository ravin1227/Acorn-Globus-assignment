    ...
    <div className=' p-2 px-8 sm:w-full md:w-5/12 bg-[#FAFAFA]'>
        <div className='px-1'>
            ...
            <div className='my-4'>
                <div className='border-l-2 border-gray-200 ml-3 mt-1 py-0 space-y-6'>
                    {job.map((item,index)=>(
                        <div className='relative' key={index}>
                            ...
                            <div className='pl-4 '>
                                <h1 className='text-[12px] text-blue-400 cursor-pointer hover:text-blue-500'>{item.title}</h1>
                ...
                <span className='flex items-center mt-4 text-[12px] text-[#50A0CE] hover:text-blue-500 cursor-pointer ml-8 gap-1'>view all jobs <IoIosArrowForward /> </span>
            </div>
        </div>
        <div>
            <div className='relative'>
                    ...
                    <ul className='divide-y divide-gray-200'>
                        {hot.slice(0,5).map((item, index)=>(
                            <div className='flex flex-row mt-2 pt-2 items-center gap-2'>
                                ...
                                <li key={index} className='text-[#50A0CE] hover:text-blue-500  text-[12px]  items-center '><a href={item.link} target="_blank" rel="noopener noreferrer" >{item.title}</a></li>
                            </div>
                        ))}
                    </ul>
                </div>
                <div className=''>
                    <span className='text-[12px] text-[#50A0CE] hover:text-blue-500  pt-4 cursor-pointer flex items-center gap-1'>
